...
class WhiskerFilter(nlg.NonlinearGaussianInitialGaussian):
    ...
    def calc_f(self, particles, u, t):
        theta, omega, period = particles[:, 0], particles[:, 1], particles[:, 2]
        period = np.abs(period)

        theta_new = (theta + omega * dt) % 2 * np.pi 
        theta_new = (theta_new + 2 * np.pi) % 2 * np.pi

        for i, angle in enumerate(theta_new):
            if angle > np.pi:
                theta_new[i] -= 2 * np.pi

        omega_new = omega - ((2 * np.pi / period) ** 2) * theta * dt

        print "theta:{}\tomega:{}\ttheta_new:{}\tomega_new:{}".format(np.mean(theta) * 180 / np.pi, np.mean(omega), np.mean(theta_new) * 180 / np.pi, np.mean(omega_new))

        return np.array([theta_new, omega_new, period]).T
